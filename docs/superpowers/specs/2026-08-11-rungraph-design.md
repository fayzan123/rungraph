...
Validated 2026-08-11 via brutal-product-analysis (two passes). Niche evidence: claude-tap 3,017★/6mo, claude-code-history-viewer 2,032★, agent-flow 1,480★/4mo. **Prior art correction (pass 2):** pradeepvairamani/claude-code-graph already does post-hoc subagent-branch graphs from `~/.claude/projects` — but as a VS Code-only sidebar, abandoned since 2026-03-15 at 3★, with no workflow-journal support, no agent-readable output, no standalone app. So the wedge is not "post-hoc is impossible elsewhere" but the unclaimed bundle: standalone npx + workflow runs + decision lineage + agent-first `--json` IR contract (no competitor has any agent-readable output). agent-flow confirmed hook-only for Claude Code (post-hoc only for Codex rollouts). Known risks: agent-flow could add a transcript adapter quickly; claude-code-graph's 3★ flop warns that post-hoc alone doesn't sell — the launch needs the live-tail GIF too; Anthropic native visualization (hedged by vendor-agnostic core); niche rides Claude Code's hype cycle. Tailwind: Dynamic Workflows (May 2026) pushes sessions toward hundreds of parallel subagents — graphs are getting too big to debug without visualization.
