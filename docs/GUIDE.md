...
again — or `all` — to clear; the filter resets on reload), a *find a run…*
box narrows it by title (case-insensitive substring; it combines with the
chip, and `Esc` clears it), and runs whose project isn't a real directory —
deleted worktrees, home-directory chats, `✦ Hermes tasks` — share one
`✦ loose runs` group.
